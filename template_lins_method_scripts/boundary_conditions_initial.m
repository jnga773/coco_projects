function [data_in, y_out] = boundary_conditions_initial(prob_in, data_in, u_in)
  ...
  x0_unstable = u_in(1:data_in.xdim);

  % Final vector of the stable manifold
  x1_stable   = u_in(data_in.xdim+1 : 2*data_in.xdim);

  % Equilibrium point
  x_ss        = u_in(2*data_in.xdim + 1 : 3*data_in.xdim);

  % System parameters
  parameters  = u_in(3*data_in.xdim + 1 : 3*data_in.xdim + 1 + data_in.pdim);
