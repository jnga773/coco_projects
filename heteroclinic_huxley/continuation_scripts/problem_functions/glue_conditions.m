function prob_out = glue_conditions(prob_in, data_in, epsilon_in)
  ...
  prob = coco_add_func(prob, 'bcs_initial', @boundary_conditions_initial, data_in, 'zero', 'uidx', ...
                       [uidx_u(maps_u.x0_idx); uidx_s(maps_s.x1_idx); uidx_u(maps_u.p_idx)], ...
                       'u0', epsilon_in);
  ...
  prob = coco_add_pars(prob, 'pars_system', uidx_u(maps_u.p_idx), {data_in.pnames{:}});

  % Define epsilon parameters
  prob = coco_add_pars(prob, 'pars_eps', uidx_eps(epsilon_idx), {'eps1', 'eps2'});

  % Define trajectory periods
  prob = coco_add_pars(prob, 'pars_T', [uidx_u(maps_u.T_idx); uidx_s(maps_s.T_idx)], {'T1', 'T2'});
