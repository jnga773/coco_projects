function plot_phase_reset_PO(run_in, save_figure)
  ...
  SP1_point = [coco_bd_val(bd, SP_labels(1), 'A'); coco_bd_val(bd, SP_labels(1), 'theta_new')];
  SP2_point = [coco_bd_val(bd, SP_labels(2), 'A'); coco_bd_val(bd, SP_labels(2), 'theta_new')];
  SP3_point = [coco_bd_val(bd, SP_labels(3), 'A'); coco_bd_val(bd, SP_labels(3), 'theta_new')];

  % Equilibrium point
  sol_ep = ep_read_solution('singularity', run_in, 1);
  x_ep   = sol_ep;
  ...
  % Plot bifurcation data
  plot(ax(1), A_data, theta_new_data, Color=colours(1, :));
  ...
  % Axis Labels
  ax(1).XAxis.Label.String = '$\theta_{\mathrm{new}}$';
  ax(1).YAxis.Label.String = '$A$';
